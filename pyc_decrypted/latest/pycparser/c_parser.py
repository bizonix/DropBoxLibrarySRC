...
class CParser(PLYParser):
    ...
    def __init__(self, lex_optimize = True, lextab = 'pycparser.lextab', yacc_optimize = True, yacctab = 'pycparser.yacctab', yacc_debug = False):
        ...
        """
        self.clex = CLexer(error_func=self._lex_error_func, type_lookup_func=self._lex_type_lookup_func)
        self.clex.build(optimize=lex_optimize, lextab=lextab)
        self.tokens = self.clex.tokens
        rules_with_opt = ['abstract_declarator',
    ...
    def _build_function_definition(self, decl, spec, param_decls, body):
        ...
        """
        declaration = c_ast.Decl(name=None, quals=spec['qual'], storage=spec['storage'], funcspec=spec['function'], type=decl, init=None, bitsize=None, coord=decl.coord)
        typename = spec['type']
    ...
    def p_function_definition_1(self, p):
        ...
        p[0] = self._build_function_definition(decl=p[1], spec=spec, param_decls=p[2], body=p[3])

    def p_function_definition_2(self, p):
        """ function_definition : declaration_specifiers declarator declaration_list_opt compound_statement
        """
        spec = p[1]
        p[0] = self._build_function_definition(decl=p[2], spec=spec, param_decls=p[3], body=p[4])
    ...
    def p_decl_body(self, p):
        ...
        if p[2] is None:
            ...
            decl = c_ast.Decl(name=None, quals=spec['qual'], storage=spec['storage'], funcspec=spec['function'], type=ty[0], init=None, bitsize=None, coord=ty[0].coord)
            decls = [decl]
        else:
            for decl, init in p[2] or []:
                if is_typedef:
                    decl = c_ast.Typedef(name=None, quals=spec['qual'], storage=spec['storage'], type=decl, coord=decl.coord)
                else:
                    decl = c_ast.Decl(name=None, quals=spec['qual'], storage=spec['storage'], funcspec=spec['function'], type=decl, init=init, bitsize=None, coord=decl.coord)
                typename = spec['type']
    ...
    def p_struct_declaration_1(self, p):
        ...
        if p[2] is not None:
            for struct_decl in p[2]:
                ...
                decl = c_ast.Decl(name=None, quals=spec['qual'], funcspec=spec['function'], storage=spec['storage'], type=struct_decl['decl'], init=None, bitsize=struct_decl['bitsize'], coord=decl_coord)
                typename = spec['type']
            ...
            decl = c_ast.Decl(name=None, quals=spec['qual'], funcspec=spec['function'], storage=spec['storage'], type=decl_type, init=None, bitsize=None, coord=self._coord(p.lineno(3)))
            decls.append(decl)
        p[0] = decls
    ...
    def p_enumerator(self, p):
        ...
        """
        if len(p) == 2:
            p[0] = c_ast.Enumerator(p[1], None, self._coord(p.lineno(1)))
        else:
            p[0] = c_ast.Enumerator(p[1], p[3], self._coord(p.lineno(1)))
    ...
    def p_direct_declarator_3(self, p):
        ...
        """
        arr = c_ast.ArrayDecl(type=None, dim=p[3], coord=p[1].coord)
        p[0] = self._type_modify_decl(decl=p[1], modifier=arr)

    def p_direct_declarator_4(self, p):
        """ direct_declarator   : direct_declarator LBRACKET TIMES RBRACKET 
        """
        arr = c_ast.ArrayDecl(type=None, dim=c_ast.ID(p[3], self._coord(p.lineno(3))), coord=p[1].coord)
        p[0] = self._type_modify_decl(decl=p[1], modifier=arr)
    ...
    def p_parameter_declaration_1(self, p):
        ...
        spec = p[1]
        decl = p[2]
        decl = c_ast.Decl(name=None, quals=spec['qual'], storage=spec['storage'], funcspec=spec['function'], type=decl, init=None, bitsize=None, coord=decl.coord)
    ...
    def p_direct_abstract_declarator_3(self, p):
        ...
        """
        p[0] = c_ast.ArrayDecl(type=c_ast.TypeDecl(None, None, None), dim=p[2], coord=self._coord(p.lineno(1)))
    ...
    def p_direct_abstract_declarator_5(self, p):
        ...
        """
        p[0] = c_ast.ArrayDecl(type=c_ast.TypeDecl(None, None, None), dim=c_ast.ID(p[3], self._coord(p.lineno(3))), coord=self._coord(p.lineno(1)))
